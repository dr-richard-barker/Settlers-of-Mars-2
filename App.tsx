...
import { GameState, Scene, StoryLogEntry, HabitatPart } from './types';
...
const App: React.FC = () => {
  ...
  const [habitatParts, setHabitatParts] = useState<HabitatPart[]>([]);
  ...
  const handleApiCall = useCallback(async (history: string, choice: string) => {
    ...
    try {
      ...
      if (scene.habitatUpdate) {
        setHabitatParts(prev => [...prev, { ...scene.habitatUpdate, id: `part_${prev.length}` }]);
      }
  ...
  const handleStartGame = useCallback(() => {
    const initialHabitat: HabitatPart = {
        id: 'lander_main',
        partType: 'CYLINDER',
    ...
    setHabitatParts([initialHabitat]);
  ...
  }, [currentScene, storyLog, handleApiCall]);
